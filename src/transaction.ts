...
export default class Transation {
    ...
    public opSet(pointer: rt.record_pointer, path: string[], args: any) {
        const operation = {
            command: 'set',
            pointer,
            path,
    ...
    public opUpdate(pointer: rt.record_pointer, path: string[], args: any) {
        const operation = {
            command: 'update',
            pointer,
            path,
    ...
    public opListRemove(pointer: rt.record_pointer, path: string[], args: { id: any }) {
        const operation = {
            command: 'listRemove',
            pointer,
            path,
    ...
    public opListBefore(pointer: rt.record_pointer, path: string[], args: { before?: any, id: any }) {
        const operation = {
            command: 'listBefore',
            pointer,
            path,
    ...
    public opListAfter(pointer: rt.record_pointer, path: string[], args: { after?: any, id: any }) {
        const operation = {
            command: 'listAfter',
            pointer,
            path,
    ...
    private async submitAll(refreshRecords: boolean = false) {
        ...
        if (refreshRecords) {
            const pointers: rt.record_pointer[] = transactions.map(transaction => transaction.operations.map(operation => operation.pointer)).flat();
    ...
    private async updateEditedInfo(operations: transaction_operation[]) {
        ...
        const editedPointerList: rt.record_pointer[] = [];
    ...
    private async getParentPage(pointer: rt.record_pointer) {
